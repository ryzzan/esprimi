...
export class CodeToAngular {
  ...
  createCode = async (object: MainInterface): Promise<BuildedCode> => {
    ...
    if (!object.module) {
      component = await this.component.createComponentCode(
        this.projectName,
        object
      );
  ...
  private createProjectName = (object: MainInterface) => {
    if (object.module) return object.module.id;

    if (object.form) return object.form.id;
