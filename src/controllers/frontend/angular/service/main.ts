...
export class CodeToAngularService {
  createServiceCode = async (
    ...
    if (object.form?.service) {
      let serviceCode = this.createFormService(
    ...
    if (object.list?.service) {
      const serviceCode = this.createService(projectName, object.list.service);

      code = code.replace("%BASE_URL%", object.list.service.baseUrl);
      code = code.replace("%SERVICES%", serviceCode);
    }
  ...
  createService = (projectName: string, service: ServiceInterface) => {
    const hasAuthorization = service.hasAuthorization
        ...
        if (Object.prototype.hasOwnProperty.call(object, key)) {
          ...
          if (key === "tabs") {
            ...
            if (tabs) {
              tabs.forEach((tab: any) => {
                selectObjectServiceCode += this.createFormService(
                  tab.elements,
                  service
                );
              });
          ...
          if (key === "select") {
            if (element?.optionsApi) {
              ...
              ${object.select.name}SelectObjectGetAll() {
                  return this._httpClient.get(
